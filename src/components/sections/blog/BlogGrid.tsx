      ...
      <div className="grid lg:grid-cols-2">
        ...
        >
          {post.coverImage ? (
            ...
            <div
              ...
              style={{
                background:
                  "linear-gradient(135deg, #1e293b 0%, #0f172a 50%, #ea580c 140%)",
              }}
            />
          )}
        ...
        <div className="relative flex flex-col justify-center px-6 py-8 sm:px-10 sm:py-12 lg:px-12">
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full bg-orange-500/15 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.16em] text-orange-300 ring-1 ring-inset ring-orange-400/30">
              Featured
            </span>
            {date ? (
            ...
            {post.tags[0] ? (
              <span className="text-xs font-medium text-slate-500">· {post.tags[0]}</span>
            ) : null}
          </div>

          <h2 className="mt-4 font-display text-2xl font-bold leading-tight tracking-tight text-white sm:text-3xl lg:text-4xl">
            <Link href={`/blog/${post.slug}`} className="transition hover:text-orange-300">
              {post.title}
            </Link>
          </h2>
...
function PostRow({ post, index }: { post: BlogPostPublic; index: number }) {
  ...
  return (
    <article className="group grid gap-5 border-b border-slate-200 py-8 last:border-0 sm:grid-cols-[auto_1fr_auto] sm:items-center sm:gap-8">
      <span className="font-display text-3xl font-bold tabular-nums text-slate-200 transition group-hover:text-orange-200 sm:w-14">
        {number}
      </span>
      ...
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500">
          ...
          {post.tags.slice(0, 2).map((tag) => (
            <span
              key={tag}
              className="rounded-md bg-orange-50 px-2 py-0.5 font-semibold text-orange-700 ring-1 ring-inset ring-orange-100"
            >
              {tag}
            </span>
          ))}
        </div>
        <h3 className="mt-2 font-display text-xl font-bold tracking-tight text-slate-900 transition group-hover:text-orange-600 sm:text-2xl">
      ...
      <Link
        href={`/blog/${post.slug}`}
        className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-700 transition group-hover:border-orange-300 group-hover:bg-orange-50 group-hover:text-orange-700"
...
function PostCard({ post }: { post: BlogPostPublic }) {
  ...
  return (
    <article className="group flex h-full flex-col">
      ...
      >
        {post.coverImage ? (
          ...
          <div
            className="absolute inset-0"
            style={{
              background: "linear-gradient(145deg, #1e293b, #0f172a 60%, #ea580c)",
            }}
          />
        )}
      ...
      <h3 className="mt-2 font-display text-lg font-bold tracking-tight text-slate-900 transition group-hover:text-orange-600">
        <Link href={`/blog/${post.slug}`}>{post.title}</Link>
      </h3>
      ...
      <Link
        href={`/blog/${post.slug}`}
        className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-orange-600 transition hover:text-orange-500"
      >
        Read article
      ...
      <section id="articles" className="bg-[#f7f5f1] py-20 lg:py-28">
        <Container className="max-w-2xl text-center">
          ...
          <p className="mt-3 text-slate-600">
            We’re drafting the next round of growth playbooks. Meanwhile, book a call and we’ll
            dig into your funnel live.
          </p>
          <Link
            href={ctaLinks.consultation}
            className="mt-8 inline-flex items-center gap-2 rounded-full bg-orange-500 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-orange-400"
          >
    ...
    <>
        ...
        <Container>
          <div className="mb-8 flex flex-col gap-2 sm:mb-10 sm:flex-row sm:items-end sm:justify-between">
            ...
            <p className="max-w-sm text-sm text-slate-500">
              {posts.length} article{posts.length === 1 ? "" : "s"} · Updated as we ship campaigns
              and learn in market.
            </p>
          </div>
      ...
      <section className="relative overflow-hidden border-t border-white/[0.06] bg-[#05080f] py-16 sm:py-20">
        <div
          ...
          style={{
            backgroundImage:
              "radial-gradient(ellipse 50% 60% at 80% 50%, rgba(249,115,22,0.18), transparent 55%)",
          }}
          aria-hidden="true"
        />
        <Container className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-xl">
            ...
            <p className="mt-3 text-sm leading-relaxed text-slate-400 sm:text-base">
              Bring your goals, traffic, and offers. We’ll map a clear growth path in one focused
              call.
            </p>
          </div>
          <Link
            href={ctaLinks.consultation}
            className={cn(
              "inline-flex shrink-0 items-center gap-2 rounded-full bg-orange-500 px-6 py-3 text-sm font-semibold text-white",
              "shadow-[0_14px_32px_-14px_rgba(249,115,22,0.85)] transition hover:bg-orange-400",
            )}
          >
            Book a free strategy call
            <ArrowRight className="h-4 w-4" aria-hidden="true" />
